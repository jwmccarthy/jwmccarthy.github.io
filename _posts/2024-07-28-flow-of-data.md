...
These transitions are collected in a loop and at each step stored in an [``EpisodicBuffer``](https://github.com/jwmccarthy/Gumbo/blob/main/gumbo/data/buffers.py) instance. This buffer holds data in PyTorch tensors via a [``TensorDataset``](https://github.com/jwmccarthy/Gumbo/blob/main/gumbo/data/datasets.py), which I have constructed as a dataclass with added tensor-like functionality. Additionally, this buffer contains episode ``Subsets``, created upon the termination of an episode, that reference particular locations in the common tensor storage. These episodes also contain auxiliary data, such as episode statistics or, importantly, terminal observations (useful for bootstraping reward).
